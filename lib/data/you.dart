...
class YouAi extends LLM {
  ...
  getResponse(
    ...
    String content = "";
    String currentModel = SettingController.to.gptModel.value;
    ...
    bool useWebSearch = SettingController.to.useWebSearch.value;
    if (useWebSearch) {
    ...
    if (SettingController.to.useStream.value) {
      Stream<OpenAIStreamChatCompletionModel> chatStream = OpenAI.instance.chat
          .createStream(
              model: currentModel,
              messages: openAIMessages,
              user: SettingController.to.youCode.value);
      chatStream.listen(
